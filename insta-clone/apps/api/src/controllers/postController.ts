...
function isLikelyCatPost(caption: string, mediaUrl: string) {
  const value = `${caption} ${mediaUrl}`.toLowerCase();
  const catHints = [
    "cat",
    "cats",
    "kitten",
  ...
  return catHints.some((hint) => value.includes(hint));
}
...
export async function createPost(req: Request, res: Response) {
  ...
  if (!isLikelyCatPost(body.caption, body.mediaUrl)) {
    throw new HttpError(400, "Only cat photos are allowed. Add a cat photo URL or cat-related caption.");
  }
